# app.py (Final, Simplified Version with st-audiorec)
...
st.write("Upload a document (.txt) to start a quiz!")
...
if st.session_state.quiz_started and st.session_state.current_q_index < len(st.session_state.questions):
    ...
    with st.form(key=f"answer_form_{st.session_state.current_q_index}"):
        ...
        with tab2:
            st.write("Click the icon to record your answer.")
            # --- THE SIMPLER AUDIO RECORDER COMPONENT ---
            wav_audio_data = None
            if AUDIO_RECORDER_AVAILABLE:
                wav_audio_data = st_audiorec()
            else:
                st.warning("Audio recorder component could not be loaded. Please ensure it's installed correctly.")

        submitted = st.form_submit_button("Submit Answer")

    # --- Processing logic is now much cleaner ---
    if submitted:
        is_correct, user_input_provided = False, False
        
        # Prioritize voice if audio was recorded
        if wav_audio_data:
        ...
        # Fallback to radio button if no audio was recorded
